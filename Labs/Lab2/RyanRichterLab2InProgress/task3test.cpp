...
#include <vector>
#include <sstream>
#include "classDeclarationTest.h"
...
int main()
{
   ...
   string key;
	while(!file.eof()){
      ...
      if (key == "ID:"){
         item1.id = stoi(line.substr(line.find(' ') + 1));
         cout << item1.id << endl;
      }
      if (key == "Units:"){
         item1.units = stoi(line.substr(line.find(' ') + 1));
         cout << item1.units << endl;
      }
      if (key == "Price:"){
         item1.price = stoi(line.substr(line.find(' ') + 1));
         cout << item1.price << endl;
      }
      if (key == "Description:"){
         item1.description = line.substr(line.find(' ') + 1);
         cout << item1.description << endl;
      }
      if (key == "TaxExempt:"){
         if (line.substr(line.find(' ') + 1) == "True"){
            item1.taxExempt = true;
         }
         else {
            item1.taxExempt = false; 
         }
         cout << item1.taxExempt << endl;
      }
   }
